...
level; instead it supplies the underlying machinery -- a model wrapper
with JIT-compiled train/predict/validate steps, batching, metrics
monitoring with optional early stopping, and Orbax-based checkpointing.
The package is deliberately task-agnostic: it works with any
"features"/"labels" pair a user's architecture function accepts, not
just bit-encoded determinants.
"""
